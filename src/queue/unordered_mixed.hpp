...
public:
	...
	std::function<void(const beam::internet::ipv4::address&, const beam::queue::common::port&)> on_disconnect;
	std::function<void(beam::message::capnproto<unreliable_msg_t>& message)> on_receive_unreliable_msg;
	std::function<void(beam::message::capnproto<reliable_msg_t>& message)> on_receive_reliable_msg;
    };
    struct perf_params
    {
	perf_params(
